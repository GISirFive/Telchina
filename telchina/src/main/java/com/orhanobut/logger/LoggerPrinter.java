...
import com.telchina.BuildConfig;
...
final class LoggerPrinter implements Printer {
	 ...
	 */
	private synchronized void log(String tag, int logType, String message) {
		...
		logTopBorder(logType, tagT);
		logHeaderContent(logType, tagT, methodCount);
		...
		if (length <= CHUNK_SIZE) {
			if (methodCount > 0) {
				logDivider(logType, tagT);
			}
			logContent(logType, tagT, message);
			logBottomBorder(logType, tagT);
			return;
		}
		if (methodCount > 0) {
			logDivider(logType, tagT);
		...
		for (int i = 0; i < length; i += CHUNK_SIZE) {
			...
			// for Android)
			logContent(logType, tagT, new String(bytes, i, count));
		}
		logBottomBorder(logType, tagT);
	}
